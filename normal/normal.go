...
func WriteIndex(allTest map[int]string) {
	outputFile, outputError := os.OpenFile("../text/index1.md", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0600)
	if outputError != nil {
		fmt.Printf("An error occurred with file opening or creation\n")
		return
	}
...
// 获取章节内容
func GetBody(title string, url string) {
	res, err := http.Get(url)
	if err != nil {
		fmt.Printf("URL获取数据 %s", err.Error())
		return
	}
	defer res.Body.Close()
	doc, err := goquery.NewDocumentFromReader(res.Body)
	if err != nil {
		fmt.Printf("demo错误 %s", err.Error())
		return
	}
...
func GetMenu() {
	...
	doc.Find("#list dl dd").Each(func(i int, s *goquery.Selection) {
		...
		GetBody(title, baseUrl+GbkToUtf(href))
